...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/role/rolegetall",//请求的url
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/user/Judgmentcode",//请求的url
			data:{method:"userCode",userCode:userCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(userCode.val()==""){
					validateTip(userCode.next(),{"color":"red"},imgNo+ "请输人用户编码",false);
				}else {
					if(data == "false"){//账号已存在，错误提示
	...
	addBtn.bind("click",function(){
		if(userCode.attr("validateStatus") != "true"){
			...
			if(confirm("是否确认提交数据")){
				var userForm=new FormData($("#userForm")[0]);
				$.ajax({
					url:"../user/add",
					type:"post",
					data:userForm,
					...
					contentType:false,//不要设置Contene-Type请求头
					"success":function (result) {
						alert(result)
						if (result=="true"){
							alert("添加用户成功！")
							location.href="../user/Pagination"
						}else {
							alert("添加用户失败！")
						}
					},
